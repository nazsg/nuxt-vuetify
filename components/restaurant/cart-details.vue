<template>
  <div>
    <div class="cart-details">
      <h4>
        Details of Order {{ $store.getters['restaurant/get_current_order'] }}
      </h4>
      ...
      <ul v-for="(cart, index) in cart_details" :key="index">
        ...
        <li>
          <cartPlus
            ...
            title="Add to cart"
            @click="
              add_to_cart([cart.id, cart.price, cart.item.slice(0, 40), 1])
            "
          />
          <cartMinus
            class="cartMinus"
...
export default {
  ...
  computed: {
    current_order() {
      const curr = this.$store.getters['restaurant/get_current_order']
      if (curr) {
        return curr
      } else {
        return ''
